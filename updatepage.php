...
// Check if the project_id parameter exists in the URL
if (isset($_GET['project_id'])) {
    ...
    echo "No project ID specified in the URL.";
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        ...
        if ($_FILES['projectImage']['error'] === UPLOAD_ERR_OK) {
            ...
            move_uploaded_file($_FILES['projectImage']['tmp_name'], $target_path);
        } else {
        ...
        $sql = "UPDATE designportfolioproject SET projectName='$pname', designCategoryID='$id', description='$des', projectImgFileName='$image' WHERE id=" . $project_id;
...
<main>
    <div class="container">
        <form method="post" action="updatepage.php" enctype="multipart/form-data">
            <input type="hidden" name="projectid" value="<?php echo isset($project_id) ? $project_id : ''; ?>">
            <label>Project name
            ...
            <label>Description
                <textarea name="description"><?php echo isset($project['description']) ? $project['description'] : ''; ?></textarea>
            </label>
